...
def check_admin_login(request):
    if request.COOKIES.get('password') == hash_string('admin@123') and request.COOKIES.get('user_type') == hash_string('admin'):
        return True
    else:
        return False
...
def index(request):
    ...
    message = 'a conference organiser app.'
    if is_admin_logged_in:
        message = "ADMIN."
    elif islogged_in:
        user_type = request.COOKIES.get('user_type')
        if user_type == hash_string('chair'):
            cra = models.Chair.objects.get(email=email)
            message = 'chair'
        elif user_type == hash_string('reviewer'):
            cra = models.Reviewer.objects.get(email=email)
            message = 'reviewer'
        elif user_type == hash_string('author'):
            cra = models.Author.objects.get(email=email)
            message = 'author'
        message = message + f" {cra.name}."
    return render(request,"index.html",{"islogged_in":islogged_in,"is_admin_logged_in":is_admin_logged_in,
                                        "user_type":request.COOKIES.get('user_type'), "message":message})

def sign_up(request):
    islogged_in = check_login(request)
    is_admin_logged_in = check_admin_login(request)
    return render(request, "sign_up.html", {"islogged_in": islogged_in, "message": "Password should contain 8-32 "
                                                                                   "characters, with atleast 1 letter, "
...
def sign_up_handle(request):
    ...
    if request.method == "POST":
        ...
        if len(password) < 8:
            return render(request, "sign_up.html", {"islogged_in": False, 'message': 'Password length is less than 8.',
                                                    "is_admin_logged_in": is_admin_logged_in,
                                                    "user_type": request.COOKIES.get('user_type')})
        elif len(password) > 32:
            return render(request, "sign_up.html", {"islogged_in": False, 'message': 'Password length is more than 32.',
                                                    "is_admin_logged_in": is_admin_logged_in,
                                                    "user_type": request.COOKIES.get('user_type')})
        else:
            strp = password.decode('utf-8')
            # print(strp)
            num_check = bool(re.search(r'\d', strp))
            char_check = bool(re.search(r'[a-zA-Z]', strp))
            spl_check = bool(re.search(r"[*.!@$%^&(){}\[\]:;<>,?/~_+\-=|]", strp))
            if not num_check:
                return render(request, "sign_up.html", {"islogged_in": False, 'message': 'Password didn\'t contain digit.',
                                                        "is_admin_logged_in": is_admin_logged_in,
                                                        "user_type": request.COOKIES.get('user_type')})
            elif not char_check:
                return render(request, "sign_up.html", {"islogged_in": False, 'message': 'Password didn\'t contain letter.',
                                                        "is_admin_logged_in": is_admin_logged_in,
                                                        "user_type": request.COOKIES.get('user_type')})
            elif not spl_check:
                return render(request, "sign_up.html", {"islogged_in": False, 'message': 'Password didn\'t contain '
                                                                                         'special character.',
                                                        "is_admin_logged_in": is_admin_logged_in,
                                                        "user_type": request.COOKIES.get('user_type')})
        ...
        if user_type == "author":
            ...
            email_send_thread = Thread(target=Utils.send_email,
                                       args=("About your registration to Congariser",
...
def login_handle(request):
    ...
    if request.method == "POST":
        ...
        if email == 'admin@example.com' and password == 'admin@123':
            ...
            hashed_admin_password = hash_string('admin@123')
            create_login_cookies(response,'admin@example.com',hashed_admin_password,'admin')
            return response
        else:
            ...
            try:
                ...
                if hashed_password != user.password:
                    return render(request, "login.html", {"islogged_in": False, 'message': 'Incorrect password.',
                                                          "is_admin_logged_in": is_admin_logged_in,
                                                          "user_type": request.COOKIES.get('user_type')})
                user_type = user.type
                try:
                    ...
                    cra = None
                    if user_type == "chair":
                        cra = models.Chair.objects.get(email=email,password=hashed_password)

                    elif user_type == "reviewer":
                        cra = models.Reviewer.objects.get(email=email, password=hashed_password)

                    elif user_type == "author":
                        cra = models.Author.objects.get(email=email, password=hashed_password)

                    message = message + f" {cra.name}."
                    context = {"islogged_in":True,"user_type":hash_string(user_type), "message": message}
                    response = render(request, "index.html",context)
                    expires = datetime.datetime.strftime(datetime.datetime.utcnow() + datetime.timedelta(seconds=max_age),
                                                         "%a, %d-%b-%Y %H:%M:%S GMT")

                    response.set_cookie(key="email", value=email, max_age=max_age, expires=expires)
                    response.set_cookie(key="password", value=cra.password, max_age=max_age, expires=expires)
                    response.set_cookie(key="user_type", value=hashed_user_type, max_age=max_age, expires=expires)
                    return response
                ...
                # Non existing user
                print(e)
                return render(request,"login.html",{"islogged_in":False,'message':'User does\'t exist.',"is_admin_logged_in":is_admin_logged_in
                    ,"user_type":request.COOKIES.get('user_type')})
...
def logout_handle(request):
    islogged_in = check_login(request)
    is_admin_logged_in = check_admin_login(request)
    response = render(request, "index.html", {"islogged_in": False,"is_admin_logged_in":False, "message": 'a conference organiser app.'})
    ...
    if request.method == "GET":
        ...
        if is_admin_logged_in:
            ...
            if user_type == "reviewer":
                ...
                reviewer.save()
                if is_accepted:
                    message = "You have been deauthorised in Congariser as a reviewer."
                else:
                    message = "You have been authorised in Congariser as a reviewer."
            elif user_type == "chair":
                chair = models.Chair.objects.get(email=user_mail)
                chair.is_accepted = (not is_accepted)
                chair.save()
                if is_accepted:
                    message = "You have been deauthorised in Congariser as a chair."
                else:
                    message = "You have been authorised in Congariser as a chair."
            email_send_thread = Thread(target=Utils.send_email, args=("Your status in Congariser.",
                                                                      message,
                                                                      user_mail,
                                                                      email_lock
                                                                      ))
